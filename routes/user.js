	...
	if (req.body._id === req.params.id || req.body.isAdmin) {
		if (req.body.password) {
			try {
				...
				console.log('getting error in bcrypt');
...
router.put('/:id/updateprofilepicture', upload.single('file'), async (req, res, next) => {
...
//update cover photo
router.put('/:id/updatecoverpicture', upload.single('file'), async (req, res, next) => {
	const userFound = await User.findById(req.params.id);
...
router.put('/:id/addNewFriends', async (req, res) => {
	// console.log('Request received to get friend ');
	...
	if (req.body.userId !== req.params.id) {
		try {
			...
			if (!user.friends.includes(currentUser._id)) {
				...
				await currentUser.updateOne({ $push: { pendingRequest: req.params.id } });
				res.status(200).json('user has been  Added in friendslist successfully');
			} else {
	...
	if (req.body.userId !== req.params.id) {
		try {
			...
			if (user.pendingRequest.includes(currentUser._id)) {
				...
				console.log('********* User after rejecting request ***********');
				console.log(currentUser);
		...
		res.status(403).json('you canot to friend reuest with urself ');
	}
});
